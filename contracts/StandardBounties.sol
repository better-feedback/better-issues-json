...
contract StandardBounties {
  ...
  */
  ...
  function issueAndContribute(
    ...
    returns(uint)
  {
    uint bountyId = issueBounty(_sender, _issuers, _approvers, _bountyId, _deadline, _token, _tokenVersion);

    contribute(_sender, _bountyId, _depositAmount);
  ...
  function performAction(
    ...
    senderIsValid(_sender)
  {
    emit ActionPerformed(_bountyId, _sender); // The _data string is emitted in an event for easy off-chain consumption
  }
  ...
  onlySubmitter(_sender, _bountyId, _fulfillmentId) // Only the original submitter of a fulfillment may update their submission
  {
    ...
    emit FulfillmentUpdated(_bountyId,
                            _fulfillmentId,
                            _fulfillers); // The _data string is emitted in an event for easy off-chain consumption
  }
  ...
  function changeData(
    ...
    onlyIssuer(_sender, _bountyId, _issuerId)
  {
    emit BountyDataChanged(_bountyId, _sender); // The new _data is emitted within an event rather than being stored on-chain for minimized gas costs
  }
  ...
  event BountyIssued(uint _bountyId, address payable _creator, address payable[] _issuers, address[] _approvers, uint _deadline, address _token, uint _tokenVersion);
  ...
  event ActionPerformed(uint _bountyId, address _fulfiller);
  event BountyFulfilled(uint _bountyId, uint _fulfillmentId, address payable[] _fulfillers, address _submitter);
  event FulfillmentUpdated(uint _bountyId, uint _fulfillmentId, address payable[] _fulfillers);
  event FulfillmentAccepted(uint _bountyId, uint  _fulfillmentId, address _approver, uint[] _tokenAmounts);
  event BountyChanged(uint _bountyId, address _changer, address payable[] _issuers, address payable[] _approvers, uint _deadline);
  ...
  event BountyDataChanged(uint _bountyId, address _changer);
